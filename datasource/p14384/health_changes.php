...
function health_changes($pid, $record_id) {
    ...
    $hc_helper = new health_changesClass($pid, $record_id);
...
class health_changesClass
{
    ...
    function findHealthChanges($survey_data, $map_data) {
        ...
        $select = array('survey_day_number','survey_date','health_change_type');
        ...
        foreach ($survey_data[$this->record_id]["repeat_instances"][$diary_event_id][""] as $rec_id => $current) {
            ...
            $survey_link = APP_PATH_WEBROOT . "DataEntry/index.php?pid=" . $this->pid . "&page=" . $diary_form . "&id=" . $rec_id;
            ...
            $health_start_date = $current['survey_date'];
